...
def normalize_deal_stage_settings(value: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for raw in value:
        ...
        # Strip leading number prefixes like "4.", "10.", "12." from labels
        import re as _re
        raw_label = str(raw.get("label") or stage_id).strip() or stage_id
        clean_label = _re.sub(r"^\d+\.", "", raw_label).strip()
        normalized.append(
            {
                "id": stage_id,
    ...
    # Inject backlog between closed_won and churned if missing from stored list
    stage_ids = [s["id"] for s in normalized]
    if "backlog" not in stage_ids:
        backlog_entry = {"id": "backlog", "label": "BACKLOG", "group": "closed", "color": "#f97316"}
        if "closed_won" in stage_ids:
            insert_at = stage_ids.index("closed_won") + 1
            normalized.insert(insert_at, backlog_entry)
        elif "churned" in stage_ids:
            normalized.insert(stage_ids.index("churned"), backlog_entry)
        else:
            normalized.append(backlog_entry)
