...
export type ContactSearchParams = {
    ...
    // Follow-up count range (inclusive) over logged calls (call_attempt_count).
    followupCountMin?: number;
    followupCountMax?: number;
    // Date-range filters (UTC ISO strings). Scheduled follow-up = next_followup_at;
    // last call = call_last_at. Either bound is optional.
    nextFollowupAfter?: string;
    nextFollowupBefore?: string;
    callLastAfter?: string;
...
export const buildContactQuery = (params: ContactSearchParams): URLSearchParams => {
  const search = new URLSearchParams();
    ...
    if (params.followupCountMin != null) search.set("call_attempt_min", String(params.followupCountMin));
    if (params.followupCountMax != null) search.set("call_attempt_max", String(params.followupCountMax));
    if (params.nextFollowupAfter) search.set("next_followup_after", params.nextFollowupAfter);
    if (params.nextFollowupBefore) search.set("next_followup_before", params.nextFollowupBefore);
    if (params.callLastAfter) search.set("call_last_after", params.callLastAfter);
...
export interface SavedView {
  id: string;
  user_id?: string | null;
  object_type: "deal" | "contact" | "company" | "prospect" | string;
  name: string;
  view_type: "table" | "kanban" | "list" | "calendar" | string;
...
export const savedViewsApi = {
  list: (objectType?: string, viewType?: string) => {
    const params = new URLSearchParams();
    if (objectType) params.set("object_type", objectType);
    if (viewType) params.set("view_type", viewType);
    const qs = params.toString();
    return request<SavedView[]>(`/api/v1/saved-views${qs ? `?${qs}` : ""}`);
  },
  create: (payload: SavedViewPayload) =>
    request<SavedView>("/api/v1/saved-views/", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  update: (id: string, payload: Partial<SavedViewPayload>) =>
    request<SavedView>(`/api/v1/saved-views/${id}`, {
      method: "PATCH",
      body: JSON.stringify(payload),
    }),
  setDefault: (id: string) =>
    request<SavedView>(`/api/v1/saved-views/${id}/default`, { method: "POST" }),
  markApplied: (id: string) =>
    request<SavedView>(`/api/v1/saved-views/${id}/applied`, { method: "POST" }),
  delete: (id: string) =>
