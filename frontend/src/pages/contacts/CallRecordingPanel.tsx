...
// Imperative handle so the parent drawer can auto-start recording when the
// pre-call countdown elapses. `startRecording` is a no-op if a recording is
// already in flight, so a rep who hit Record manually during the countdown
// won't get it restarted.
export interface CallRecordingPanelHandle {
  startRecording: () => void;
  isRecording: () => boolean;
}
...
export const CallRecordingPanel = forwardRef<CallRecordingPanelHandle, {
  ...
  // Mirror phase into a ref so the imperative startRecording() — called by the
  // parent drawer when the pre-call countdown elapses — can guard against
  // double-starting without closing over a stale `phase`.
  const phaseRef = useRef(phase);
  useEffect(() => { phaseRef.current = phase; }, [phase]);
  useImperativeHandle(ref, () => ({
    startRecording: () => {
      if (phaseRef.current === "idle" || phaseRef.current === "failed") void start();
    },
