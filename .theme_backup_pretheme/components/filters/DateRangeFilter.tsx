...
export default function DateRangeFilter({
  ...
  const buttonStyle: CSSProperties = {
    ...
    height: 42,
    borderRadius: 12,
    border: active ? "1.5px solid #ffc9b4" : "1px solid #d9e1ec",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      <div ref={ref} style={{ position: "relative" }}>
        <button type="button" onClick={() => setOpen((current) => !current)} style={buttonStyle}>
          <CalendarRange size={14} style={{ position: "absolute", left: 11, top: "50%", transform: "translateY(-50%)", color: active ? "#ff6b35" : "#7f8fa5" }} />
          {displayLabel}
          <ChevronDown size={13} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: "#7f8fa5" }} />
        </button>
        {open && (
          <div style={{ position: "absolute", top: "calc(100% + 6px)", left: 0, zIndex: 20, minWidth: 260, borderRadius: 14, border: "1px solid #dbe6f2", background: "#fff", boxShadow: "0 18px 36px rgba(15,23,42,0.14)", padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              ...
              {active && (
                <button type="button" onClick={() => onChange({ from: "", to: "" })} style={{ border: "none", background: "transparent", color: "#ff6b35", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>
                  Clear
                </button>
              )}
            </div>
                ...
                {presets.map((preset) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        border: isOn ? "1px solid #ffc9b4" : "1px solid #e2eaf2",
                        background: isOn ? "#fff3ec" : "#f8fafc",
                        color: isOn ? "#b85024" : "#4d6178",
                        borderRadius: 999,
                        padding: "4px 10px",
                        fontSize: 11.5,
                        fontWeight: 700,
